...
const UI = {
  ...
  renderNextFixture(){
    ...
    // Get opponent — use matchday index counting from beginning of matchdays array
    const opponents=G.league.teams.filter(t=>!t.isPlayer);
    const alreadyPlayed=(G.league.matchdays||[]).filter(d=>d<=day).length;
    const opp=opponents.length>0?opponents[alreadyPlayed%opponents.length]?.name||'Opponents':'Opponents';
    ...
    el.innerHTML=`<div class="fixture-pill">
      <span class="fix-badge">⚽ NEXT FIXTURE</span>
      <span class="fix-teams">${G.club.name} <em style="color:var(--text-muted);font-style:normal;margin:0 3px;">vs</em> ${next.isCup?'Cup Opponents':opp}</span>
