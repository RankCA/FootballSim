...
const App = {
  ...
  skipDays(n){
    ...
    for(let i=0;i<n;i++){
      ...
      G.season.dayOfSeason++;
      if(result.blockingEvent){App.renderDashboard();UI.showBlockingEvent(result.blockingEvent);return;}
    }
    if(G.season.dayOfSeason>=G.season.totalDays)endSeason();
    else{
      const evEl=document.getElementById('lastDayEvents');
...
function renderStepsBar(){
  ...
  document.getElementById('steps-bar').innerHTML=labels.map((l,i)=>{
    ...
    return`<div class="${cls}"><div class="step-dot">${n<creationStep?'✓':n}</div></div>${i<3?'<div class="step-line"></div>':''}`;
  }).join('');
}
...
function stepReviewHTML(){
  ...
  return`<div class="section-label">Step 4 of 4 — Review</div><h2>Ready, Player One?</h2>
    ...
    <div class="btn-row"><button class="btn btn-secondary" onclick="creationStep=3;renderCreationStep()">← Edit</button><button class="btn btn-primary" style="font-size:15px;" onclick="beginCareer()">⚽ Begin Career</button></div>`;
}
