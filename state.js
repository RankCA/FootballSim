...
function resolveEvent(fnName){
  ...
  const actions={
    ...
    acceptRecordTransfer:()=>{
      ...
      // New manager on transfer
      G.manager={name:E.pick(MANAGER_NAMES),title:E.pick(MANAGER_TITLES),teamAvgOVR:E.rand(72,90)};
      G.careerStats.highestLeague=Math.min(G.careerStats.highestLeague,tier);
      // Keep existing league progress (don't fully reset — only update team list)
    ...
    careerEndingInjury:()=>{
      G.careerInjuryMonthsLeft=8;G.injuryDaysLeft=0;
    ...
    retireInjury:()=>{
      ...
      setTimeout(()=>App.confirmRetirement(),500);
    },
    tragicDeath:()=>{
      saveToHOF();
      localStorage.removeItem('propath3_save');
      closeModal();
      App.goTo(0);
      showToast('🕯️ A career that will never be forgotten.','');
    },
    wcPlay:()=>{
...
function acceptTransferOffer(offerId){
  ...
  // FIX: New manager on transfer
  G.manager={name:E.pick(MANAGER_NAMES),title:E.pick(MANAGER_TITLES),teamAvgOVR:E.rand(60,82)};
  ...
  G.pendingTransferOffers=[];
  // FIX: Preserve existing league progress — only update teams, not reset matchdays/scores
...
function requestNewContract(){
  ...
  const day=G.season.dayOfSeason;
  // FIX: 6-month cooldown (~180 days) after signing a contract
  const daysSinceSigned=day-(G.club.contractSignedDay||-999);
  if(daysSinceSigned<180&&daysSinceSigned>=0){
  ...
  <div class="event-choices">
    ...
    <button class="event-choice" onclick="closeModal()"><div class="ec-label" style="color:var(--text)">🤔 Think about it</div><div class="ec-outcome" style="color:var(--text-dim)">Come back later.</div></button>
  </div>`);
}

function signNewContract(salary,years){
  G.weeklySalary=salary;G.club.contractYears=years;G.club.contractSignedDay=G.season.dayOfSeason;
...
function startNewSeason(){
  ...
  G.cups=E.scheduleCups(G.club.tier);G.manager.teamAvgOVR=E.rand(58,76);
