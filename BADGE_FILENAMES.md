# Badge Filenames — ProPath

All files are **PNG**, placed in the folder shown. Add the file to its respective league folder, ensuring it has the correct filename & the game will start using the badge automatically.

---

## 🏴󠁧󠁢󠁥󠁮󠁧󠁿 Premier League → `badges/prem/`

| Club Name | Filename |
|-----------|----------|
| Arsenal | `arsenal.png` |
| Chelsea | `chelsea.png` |
| Liverpool | `liverpool.png` |
| Manchester City | `manchester_city.png` |
| Manchester United | `manchester_united.png` |
| Tottenham Hotspur | `tottenham_hotspur.png` |
| Newcastle United | `newcastle_united.png` |
| Aston Villa | `aston_villa.png` |
| West Ham United | `west_ham_united.png` |
| Brighton & Hove Albion | `brighton_and_hove_albion.png` |
| Brentford FC | `brentford_fc.png` |
| Crystal Palace | `crystal_palace.png` |
| Everton FC | `everton_fc.png` |
| Fulham FC | `fulham_fc.png` |
| Leicester City | `leicester_city.png` |
| Nottingham Forest | `nottingham_forest.png` |
| Wolverhampton W | `wolverhampton_w.png` |
| Bournemouth AFC | `bournemouth_afc.png` |
| Ipswich Town | `ipswich_town.png` |
| Sheffield United | `sheffield_united.png` |

---

## 🏴󠁧󠁢󠁥󠁮󠁧󠁿 Championship → `badges/champ/`

| Club Name | Filename |
|-----------|----------|
| Southampton | `southampton.png` |
| Leeds United | `leeds_united.png` |
| Burnley | `burnley.png` |
| Middlesbrough | `middlesbrough.png` |
| Coventry City | `coventry_city.png` |
| Preston NE | `preston_ne.png` |
| Stoke City | `stoke_city.png` |
| Millwall | `millwall.png` |
| Swansea City | `swansea_city.png` |
| Birmingham City | `birmingham_city.png` |
| Blackburn Rovers | `blackburn_rovers.png` |
| Blackpool FC | `blackpool_fc.png` |
| Cardiff City | `cardiff_city.png` |
| Huddersfield Town | `huddersfield_town.png` |
| Hull City | `hull_city.png` |
| Luton Town | `luton_town.png` |
| Norwich City | `norwich_city.png` |
| QPR | `qpr.png` |
| Sheffield Wednesday | `sheffield_wednesday.png` |
| Sunderland AFC | `sunderland_afc.png` |
| Watford FC | `watford_fc.png` |
| West Brom | `west_brom.png` |
| Bristol City | `bristol_city.png` |
| Nottm Forest | `nottm_forest.png` |

---

## 🏴󠁧󠁢󠁥󠁮󠁧󠁿 League One → `badges/l1/`

| Club Name | Filename |
|-----------|----------|
| Portsmouth | `portsmouth.png` |
| Charlton Athletic | `charlton_athletic.png` |
| Lincoln City | `lincoln_city.png` |
| Shrewsbury Town | `shrewsbury_town.png` |
| Peterborough Utd | `peterborough_utd.png` |
| Oxford United | `oxford_united.png` |
| Exeter City | `exeter_city.png` |
| Derby County | `derby_county.png` |
| Bristol Rovers | `bristol_rovers.png` |
| Barnsley FC | `barnsley_fc.png` |
| Bolton Wanderers | `bolton_wanderers.png` |
| Burton Albion | `burton_albion.png` |
| Cambridge United | `cambridge_united.png` |
| Cheltenham Town | `cheltenham_town.png` |
| Fleetwood Town | `fleetwood_town.png` |
| Leyton Orient | `leyton_orient.png` |
| Plymouth Argyle | `plymouth_argyle.png` |
| Reading FC | `reading_fc.png` |
| Rotherham United | `rotherham_united.png` |
| Wigan Athletic | `wigan_athletic.png` |
| Wycombe Wanderers | `wycombe_wanderers.png` |
| Stevenage FC | `stevenage_fc.png` |

---

## 🏴󠁧󠁢󠁥󠁮󠁧󠁿 League Two → `badges/l2/`

| Club Name | Filename |
|-----------|----------|
| Grimsby Town | `grimsby_town.png` |
| Crawley Town | `crawley_town.png` |
| Swindon Town | `swindon_town.png` |
| Bradford City | `bradford_city.png` |
| Barrow AFC | `barrow_afc.png` |
| Doncaster Rovers | `doncaster_rovers.png` |
| AFC Wimbledon | `afc_wimbledon.png` |
| Colchester United | `colchester_united.png` |
| Salford City | `salford_city.png` |
| Accrington Stanley | `accrington_stanley.png` |
| Carlisle United | `carlisle_united.png` |
| Crewe Alexandra | `crewe_alexandra.png` |
| Forest Green Rovers | `forest_green_rovers.png` |
| Gillingham FC | `gillingham_fc.png` |
| Mansfield Town | `mansfield_town.png` |
| MK Dons | `mk_dons.png` |
| Newport County | `newport_county.png` |
| Northampton Town | `northampton_town.png` |
| Port Vale | `port_vale.png` |
| Rochdale AFC | `rochdale_afc.png` |
| Stockport County | `stockport_county.png` |
| Tranmere Rovers | `tranmere_rovers.png` |

---

## 🏴󠁧󠁢󠁥󠁮󠁧󠁿 National Youth League → `badges/l5/`

| Club Name | Filename |
|-----------|----------|
| Stevenage Youth | `stevenage_youth.png` |
| Harrogate Town Academy | `harrogate_town_academy.png` |
| Fleetwood Town Academy | `fleetwood_town_academy.png` |
| Morecambe Academy | `morecambe_academy.png` |
| Aldershot Town | `aldershot_town.png` |
| Barnet FC | `barnet_fc.png` |
| Boreham Wood | `boreham_wood.png` |
| Dover Athletic | `dover_athletic.png` |
| Woking FC | `woking_fc.png` |
| Bromley Academy | `bromley_academy.png` |
| Dagenham & Redbridge | `dagenham_and_redbridge.png` |
| Ebbsfleet United | `ebbsfleet_united.png` |
| Eastleigh FC | `eastleigh_fc.png` |
| FC Halifax Town | `fc_halifax_town.png` |
| Hartlepool Academy | `hartlepool_academy.png` |
| Maidenhead United | `maidenhead_united.png` |
| Oldham Athletic | `oldham_athletic.png` |
| Solihull Moors | `solihull_moors.png` |
| Torquay United | `torquay_united.png` |
| Wrexham Academy | `wrexham_academy.png` |
| York City Academy | `york_city_academy.png` |
| Kidderminster Harriers | `kidderminster_harriers.png` |

---

## 🇪🇸 La Liga → `badges/laliga/`

| Club Name | Filename |
|-----------|----------|
| Real Madrid | `real_madrid.png` |
| FC Barcelona | `fc_barcelona.png` |
| Atlético de Madrid | `atletico_de_madrid.png` |
| Sevilla FC | `sevilla_fc.png` |
| Real Betis | `real_betis.png` |
| Real Sociedad | `real_sociedad.png` |
| Athletic Club | `athletic_club.png` |
| Villarreal CF | `villarreal_cf.png` |
| Valencia CF | `valencia_cf.png` |
| Celta Vigo | `celta_vigo.png` |
| Rayo Vallecano | `rayo_vallecano.png` |
| Osasuna | `osasuna.png` |
| Getafe CF | `getafe_cf.png` |
| Mallorca | `mallorca.png` |
| Girona FC | `girona_fc.png` |
| Deportivo Alavés | `deportivo_alaves.png` |
| Cádiz CF | `cadiz_cf.png` |
| Almería | `almeria.png` |
| Las Palmas | `las_palmas.png` |
| Espanyol | `espanyol.png` |

---

## 🇮🇹 Serie A → `badges/seriea/`

| Club Name | Filename |
|-----------|----------|
| Inter Milan | `inter_milan.png` |
| AC Milan | `ac_milan.png` |
| Juventus | `juventus.png` |
| Napoli | `napoli.png` |
| Lazio | `lazio.png` |
| AS Roma | `as_roma.png` |
| Atalanta | `atalanta.png` |
| Fiorentina | `fiorentina.png` |
| Bologna | `bologna.png` |
| Torino | `torino.png` |
| Monza | `monza.png` |
| Udinese | `udinese.png` |
| Sassuolo | `sassuolo.png` |
| Empoli | `empoli.png` |
| Lecce | `lecce.png` |
| Salernitana | `salernitana.png` |
| Cagliari | `cagliari.png` |
| Frosinone | `frosinone.png` |
| Verona | `verona.png` |
| Genoa | `genoa.png` |

---

## Tips
- Badges don't need to be exact size. They're rescaled to fit (20px in table, 20px in fixture panel, 20px in match HUD)
- Square PNGs work best (e.g. 64×64, 128×128, 512×512)
- Transparent background recommended
- If a file is missing, the emoji badge shows instead
- You can add badges one club at a time; others will keep using emoji until you add theirs
